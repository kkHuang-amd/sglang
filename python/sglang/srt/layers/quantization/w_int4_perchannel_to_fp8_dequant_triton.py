...
def wint4_perchannel_to_fp8_dequantize_triton(
    ...
    E = qweight.shape[0]
    K = qweight.shape[1]
    M_8 = qweight.shape[2]  # M//8
    M = M_8 * 8
    assert weight_scale1.shape[1] == K 
    ...
    tmp_result = torch.zeros((K, M), device=qweight.device, dtype=torch.float8_e4m3fnuz)
    grid = (K,)
    ...
    result = tmp_result[None,:].to(torch.bfloat16)
    ...
    for i in range (1,E):
        ...
        result = torch.cat((result, tmp_result[None,:].to(torch.bfloat16)), 0)

    return result.to(torch.float8_e4m3fnuz)
